...
本项目已提供 GitHub Actions 工作流：`.github/workflows/android-apk.yml`，会先把网页资源整理到 `www/`（Capacitor 的 `webDir`）、在 CI 中强制写入有效的 `capacitor.config.json`，并配置 Java 17 + npm cache。若仓库已包含 `android/` 工程，workflow 会自动跳过 `cap add android`，避免重复添加报错。
